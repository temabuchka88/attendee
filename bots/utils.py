...
def generate_aggregated_utterances(recording):
    utterances_sorted = sorted(recording.utterances.filter(recording_artifact=None).all(), key=lambda x: x.timestamp_ms)
...
def generate_failed_utterance_json_for_bot_detail_view(recording):
    failed_utterances = recording.utterances.filter(recording_artifact=None).filter(failure_data__isnull=False).order_by("timestamp_ms")[:10]
